...
CHANNELS = ['cy3', 'cy5'] # channels to stack
...
def imlist_to_df(imlist: List) -> pd.DataFrame:
...
def get_stack_num(path):
    ...
    sample = [f for f in os.listdir(path) if f.endswith(
        '.tif') and not f.startswith('.')][0]
...
def stack_cycle(in_directory, out_directory, cycle):
    ...
    for chn in CHANNELS:
        if glob(os.path.join(in_directory, cycle, f'*-{chn}-*')):
            chn_info[chn] = imlist_to_df([f for f in os.listdir(os.path.join(
                in_directory, cycle)) if chn in f and not f.startswith('.')])
            ...
            for tile in tqdm(captured_tiles, desc=f'Cycle {cyc}, Channel {chn}'):
                z_stack_names = [os.path.join(
                    in_directory, cycle, f'C{cyc:03d}-T{tile:04d}-{chn}-Z{z:03d}.tif') for z in range(stack_num)]
                eng.fstack_write(z_stack_names, os.path.join(
                    out_directory, f'cyc_{cyc}_{chn}', f'FocalStack_{tile:03d}.tif'), nargout=0)
    eng.quit()
...
def main():
    ...
    while True:
        cycles = [f for f in os.listdir(src_directory) if re.match(
            r'^cyc_([0-9]+)$', f) and f != 'cyc_0']
        print("Cycles", '\n'+'\t'.join(cycles))
        for cyc in cycles:
            if glob(os.path.join(src_directory, cyc, 'finish.txt')) and not glob(os.path.join(dest_directory, f'{cyc}_*')):
                wait = 0
